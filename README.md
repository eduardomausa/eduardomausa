## Hi there, I'm Eduardo 👋

💻 **Full Stack Developer | React | React Native | Node.js | TypeScript**  
📍 Based in Florianópolis, Brazil

## 🚀 About Me
- 👨‍💻 5 years of professional experience in software development  
- 💳 Contributed to building a **Fintech Digital Bank App** in Brazil, helping thousands of users manage their finances securely and efficiently  
- 🍕 Helped develop a **large-scale eCommerce platform** for one of the world’s biggest pizzerias 
- 🌱 Currently expanding my knowledge in **AI & Data Science** to bridge the gap between frontend development and data-driven solutions  
- 🏋️‍♂️ Passionate about **fitness, running, and surfing**, always seeking balance between technology and an active lifestyle  


## 🛠️ Tech Stack
- **Frontend:** React, Next.js, React Native, TypeScript, Redux, XState, MobX, Tailwind, Styled Components
- **Backend:**  Node.js, Express, GraphQL, Firebase, REST APIs, Microservices, SOLID, DDD, Postgres, DynamoDB
- **Cloud:** AWS, Lambda, SQS, SNS, EventBridge, DynamoDB, API Gateway
- **Testing:** Jest, React Testing Library, Playwright  
- **Tools:** Communication, Collaboration, Teamwork, Detail-oriented, Ownership
- **Other Interests:** Microservices, Event-driven Architecture, Machine Learning, AI

## Find more about me
- 🌐 [Website](https://www.eduardomausa.com/)  
- 💼 [LinkedIn](https://www.linkedin.com/in/eduardo-mausa/)  
